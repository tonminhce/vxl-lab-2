...
/* USER CODE BEGIN PV */
GPIO_TypeDef * SWITCH_PORT = GPIOA;
/* USER CODE END PV */
...
uint16_t SWITCH_PIN[4] = {GPIO_PIN_10, GPIO_PIN_11, GPIO_PIN_12, GPIO_PIN_13};
int CLOCK_NUMBER[4] = {1,2,3,0};
...
void segmentLed(int n, int select){
  ...
  if (select != 0)
	  HAL_GPIO_WritePin(GPIOA, SWITCH_PIN[0], 1);
  if (select != 1)
	  HAL_GPIO_WritePin(GPIOA, SWITCH_PIN[1], 1);
  if (select != 2)
	  HAL_GPIO_WritePin(GPIOA, SWITCH_PIN[2], 1);
  if (select != 3)
	  HAL_GPIO_WritePin(GPIOA, SWITCH_PIN[3], 1);
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
	...
	segmentLed( CLOCK_NUMBER[select], select);
}
/* USER CODE END 0 */
...
static void MX_GPIO_Init(void)
{
  ...
  HAL_GPIO_WritePin(GPIOA, GPIO_PIN_4|GPIO_PIN_10|GPIO_PIN_11|GPIO_PIN_12
